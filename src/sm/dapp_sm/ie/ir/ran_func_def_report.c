...
void free_ran_func_def_report_dapp_sm(ran_func_def_report_dapp_sm_t* src)
{
  ...
  for (size_t i = 0; i < src->sz_seq_report_sty; ++i) {
    free_seq_report_sty_dapp_sm(&src->seq_report_sty[i]);
  }
...
bool eq_ran_func_def_report_dapp_sm(ran_func_def_report_dapp_sm_t const* m0, ran_func_def_report_dapp_sm_t const* m1)
{
  ...
  for (size_t i = 0; i < m0->sz_seq_report_sty; ++i) {
    if (eq_seq_report_sty_dapp_sm(&m0->seq_report_sty[i], &m1->seq_report_sty[i]) == false)
      return false;
  }
...
ran_func_def_report_dapp_sm_t cp_ran_func_def_report_dapp_sm(ran_func_def_report_dapp_sm_t const* src)
{
  ...
  dst.seq_report_sty = calloc(dst.sz_seq_report_sty, sizeof(seq_report_sty_dapp_sm_t));
  assert(dst.seq_report_sty != NULL && "Memory exhausted");

  for (size_t i = 0; i < src->sz_seq_report_sty; ++i) {
    dst.seq_report_sty[i] = cp_seq_report_sty_dapp_sm(&src->seq_report_sty[i]);
  }
