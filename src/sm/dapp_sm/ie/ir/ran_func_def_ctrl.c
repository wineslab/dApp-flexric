...
void free_ran_func_def_ctrl_dapp_sm(ran_func_def_ctrl_dapp_sm_t* src)
{
  ...
  for (size_t i = 0; i < src->sz_seq_ctrl_style; ++i) {
    free_seq_ctrl_style_dapp_sm(&src->seq_ctrl_style[i]);
  }
...
bool eq_ran_func_def_ctrl_dapp_sm(ran_func_def_ctrl_dapp_sm_t const* m0, ran_func_def_ctrl_dapp_sm_t const* m1)
{
  ...
  for (size_t i = 0; i < m0->sz_seq_ctrl_style; ++i) {
    if (eq_seq_ctrl_style_dapp_sm(&m0->seq_ctrl_style[i], &m1->seq_ctrl_style[i]) == false)
      return false;
  }
...
ran_func_def_ctrl_dapp_sm_t cp_ran_func_def_ctrl_dapp_sm(ran_func_def_ctrl_dapp_sm_t const* src)
{
  ...
  dst.seq_ctrl_style = calloc(dst.sz_seq_ctrl_style, sizeof(seq_ctrl_style_dapp_sm_t));
  assert(dst.seq_ctrl_style != NULL && "Memory exhausted");

  for (size_t i = 0; i < dst.sz_seq_ctrl_style; ++i) {
    dst.seq_ctrl_style[i] = cp_seq_ctrl_style_dapp_sm(&src->seq_ctrl_style[i]);
  }
